...
def prepare_data(root_dir, embedding_file, pepGraph_fname, pdb, chain,
    ...
    # graph process
    pepGraph = torch.load(pepGraph_file)
    print('seq_embedding:', input_array.shape)
    print('pepGraph.x:', pepGraph.x.shape)
...
if __name__ == "__main__":
    ...
    result_dir = '/home/lwang/models/HDX_LSTM/results/GNN_type/240229_localGraph'
    ...
    torch.save(input_data, f'{result_dir}/input_data.pt')    
    #input_data = torch.load(f'{result_dir}/input_data.pt')
...
'''
    ...
    train_set, val_set = train_test_split(input_data, test_size=0.2, random_state=42)
    ...
    y_true, y_pred, pep_centers = test_model(model, val_loader, device)
    plt.figure()
