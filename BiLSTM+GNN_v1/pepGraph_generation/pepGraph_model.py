...
class GCN(nn.Module):
    def __init__(self, args):
        ...
        gcn_layers = []
        GNN_layer = {'GCN': GCNConv, 'GraphSAGE': SAGEConv, 'GAT': GATConv}
        if args['GNN_type'] == 'GCN':
            gcn_layers.append(GCNConv(self.in_dim, self.hidden_dim))
            ...
            gcn_layers.append(SAGEConv(self.in_dim, self.hidden_dim, aggr = 'mean'))
            for _ in range(self.num_GNN_layers - 2):
            ...
            gcn_layers.append(GATConv(self.in_dim, self.hidden_dim, heads=8, dropout=self.drop_rate))
            for _ in range(self.num_GNN_layers - 2):
                gcn_layers.append(GATConv(self.hidden_dim*8, self.hidden_dim, heads=8))
            gcn_layers.append(GATConv(self.hidden_dim*8, self.module_out_dim, heads=8))
        self.gcn_layers = nn.ModuleList(gcn_layers)

        self.drop_out = nn.Dropout(self.drop_rate)
        self.bn = tg.nn.global_mean_pool
        self.GAT_linear = nn.Linear(self.module_out_dim*8, self.module_out_dim)
        self.fc = nn.Linear(self.module_out_dim, self.module_out_dim)
...
class MixGCNBiLSTM(nn.Module):
    ...
    def forward(self, graph_data):
        seq_embedding = graph_data['seq_embedding'].reshape(-1, 1, 30, 48) #[batch, channel, max_len, feature_dim]
