...
def AF_cutter(args):
    ...
    def add_header_end_to_pdb(file_path, header_line, end_line):
        with open(file_path, 'r+') as file:
            content = file.read()
            file.seek(0, 0)
            file.write(header_line.rstrip('\r\n') + '\n' + content + end_line)
    ...
    for pdb in filepath_list:
        ...
        for chain_id, chain_df in pdb_df_dict.items():
            ...
            add_header_end_to_pdb(pdb_sep, Header_line, End_line)
            ...
            pdb_df_dict_groupby[chain_id] = chain_df.groupby('residue_number').agg(custom_mode)
