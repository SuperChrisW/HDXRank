...
"""
preprocess.py
Prepare input files for Frodan
"""
...
import  python_scripts.map_atomsChains
...
def usage():
	print "Usage: %s  -i initial.pdb [-t final.pdb] -o options.xml [-c chainc.txt] [-nohet]"%(sys.argv[0])
...
def main():
	...
	Command: preprocess.py -i initial.pdb -t final.pdb -o options.xml  -c chain.txt -nohet
	"""
	setEnvVariable()
	...
	if "-t" in comline_arg:
		...
		if "map" in targ_files:
			if "-c" in comline_arg: # If user provides file with chains mapping
				atom_matchfile = python_scripts.map_atomsChains.mapAtoms(file_list, comline_arg["-c"])
			else:
