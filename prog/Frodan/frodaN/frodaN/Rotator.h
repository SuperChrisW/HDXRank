...
public:
  ...
  void rotate( const Vec3& baseVec, Vec3& rotatedVec ) {
    ...
    Vec3 rotatedVec_;
    rotatedVec_.x = M11*baseVec.x + M12*baseVec.y + M13*baseVec.z;
    rotatedVec_.y = M21*baseVec.x + M22*baseVec.y + M23*baseVec.z;
    rotatedVec_.z = M31*baseVec.x + M32*baseVec.y + M33*baseVec.z;
    rotatedVec = rotatedVec_;
  }

  void rotateAboutCenter( const Vec3& absoluteVec, const Vec3& centerOfRotation, Vec3& rotatedVec ) {
    Vec3 vec = absoluteVec;
    vec -= centerOfRotation;
    rotate( vec, rotatedVec );
    rotatedVec += centerOfRotation;
  }
