...
RigidUnitSystem::RigidUnitSystem() :
  ...
  rigidUnitGeometryChanged( false )
{
}
...
void RigidUnitSystem::buildRigidUnitFromAbsolutePositions( int ru ) {
  data.rotors[ru].x = data.rotors[ru].y = data.rotors[ru].z = 0;
  basePositionsChanged = true;
  rigidUnitGeometryChanged = true;
...
void RigidUnitSystem::setRigidUnit( int ru, const vector<Vec3>& absolutePositions ) {
  const vector<int>& ruplist = getRUPlistFromRU( ru );
  for ( size_t i = 0; i < ruplist.size(); i++ ) {
    data.absolutePositions[ ruplist[i] ] = absolutePositions[i];
  }
...
void RigidUnitSystem::setRigidUnit( int ru, const vector<Vec3>& absolutePositions,
  ...
  const vector<int>& ruplist = getRUPlistFromRU( ru );
  for ( size_t i = 0; i < ruplist.size(); i++ ) {
    data.absolutePositions[ ruplist[i] ] = absolutePositions[ indices[i] ];
  }
  buildRigidUnitFromAbsolutePositions( ru );
  absolutePositionsChanged = true;
}

void RigidUnitSystem::setRigidUnits( const vector<Vec3>& absolutePositions ) {
  data.absolutePositions = absolutePositions;
...
void RigidUnitSystem::configureRigidUnits( const vector< vector<int> >& RUtoPlist_, int nP ) {
  lookup.setLookup_RUtoPlist( RUtoPlist_, nP );
  data.setSizes( nP, lookup.nRigidUnits(), lookup.nRigidUnitPoints() );
}

void RigidUnitSystem::setRigidUnitsFromMeanPositions( const vector<Vec3>& meanPositions ) {
  ...
  for ( int rup = 0; rup < nRUP; rup++ ) {
    ...
    data.absolutePositions[rup] = meanPositions[p];
  }
  absolutePositionsChanged = true;
...
void RigidUnitSystem::reinitializeToNewMeanPositions(
           ...
           const vector<Vec3>& meanPositions_ )
{
  configureRigidUnits( RUtoPlist_, meanPositions_.size() );
  setRigidUnitsFromMeanPositions( meanPositions_ );
}
...
void RigidUnitSystem::updateAbsolutePositions( int ru ) {
  Rotator rotator( data.rotors[ru] );
...
void RigidUnitSystem::globalTranslate( const Vec3& deltaCenter ) {
  size_t nRU = lookup.nRigidUnits();
  for ( size_t ru = 0; ru < nRU; ru++ ) {
    addToCenter( ru, deltaCenter );
  }
}

void RigidUnitSystem::globalRotate( const Vec3& rotor, const Vec3& centerOfRotation ) {
  size_t nRU = lookup.nRigidUnits();
  Rotator rotator( rotor );
  for ( size_t ru = 0; ru < nRU; ru++ ) {
    rotate( ru, rotator, centerOfRotation );
  }
}
