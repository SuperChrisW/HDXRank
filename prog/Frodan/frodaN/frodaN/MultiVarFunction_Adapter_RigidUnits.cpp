...
void MultiVarFunction_Adapter_RigidUnits::getPreconditionedNegGrad( std::vector<double> &s, bool &isPreconditioned )
{
  ...
  size_t i = 0;
  for ( size_t ru=0;  ru < nRU; i += 3, ru++ ) {
    if ( abs(d2V_dQ2_diagonal->centersRU[ru].x) > 1e-6 )
      s[i] /= d2V_dQ2_diagonal->centersRU[ru].x;
    if ( abs(d2V_dQ2_diagonal->centersRU[ru].y) > 1e-6 )
      s[i+1] /= d2V_dQ2_diagonal->centersRU[ru].y;
    if ( abs(d2V_dQ2_diagonal->centersRU[ru].z) > 1e-6 )
  ...
  for ( size_t ru=0;  ru < nRU; i += 3, ru++ ) {
    ...
    if ( abs(d2V_dQ2_diagonal->rotorsRU[ru].x) > 1e-6 )
      s[i] = s[i]/(d2V_dQ2_diagonal->rotorsRU[ru].x/rotorScaleFactor+10.0);
    if ( abs(d2V_dQ2_diagonal->rotorsRU[ru].y) > 1e-6 )
      s[i+1] = s[i+1]/(d2V_dQ2_diagonal->rotorsRU[ru].y/rotorScaleFactor+10.0);
    if ( abs(d2V_dQ2_diagonal->rotorsRU[ru].z) > 1e-6 )
      s[i+2] = s[i+2]/(d2V_dQ2_diagonal->rotorsRU[ru].z/rotorScaleFactor+10.0);
  }
