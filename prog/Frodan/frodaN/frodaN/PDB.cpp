...
void PDBAtomLine::parseAtomLine( const string& line ) {
  stringstream ss;
  string tempString;
  int tempInt;
  double tempDouble;
...
string PDBAtomLine::strippedName() const {
  size_t istart = name.find_first_not_of(' ');
  if ( istart == name.npos ) return "";
  size_t iend = name.find_last_not_of(' ');
  size_t n = iend - istart + 1;
  return name.substr( istart, n );
}


void ConectRecord::parseLine( const string& line ) {
  stringstream ss;
  if ( line.size() >= 16 ) {
    ss << line.substr( 6, 5 );
    ss >> baseAtom;
...
ostream& operator<< (ostream& os, const PDBAtomLine& atomLine ) {
  ...
  os << setw(3) << atomLine.resName; // 17-19
  os << " "; // 20
...
void PDB::read( string filename ) {
  ...
  vector<PDBAtomLine>(atomLines).swap(atomLines);
  vector<LineInfo>(lookupLineInfo).swap(lookupLineInfo);
  vector<ConectRecord>(conectRecords).swap(conectRecords);
