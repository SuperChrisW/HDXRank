      ...
      double fa, double fx_, double fb )
{
  ...
  if ( fx > fa || fx > fb || a >= x || x >= b ) {
    std::cout << "Error in LineMinimizer_BrentMethod: input values do not form a valid bracket " << std::endl;
    exit( 0 );
  }
  ...
  for ( iteration = 0; iteration < maxIterations; iteration++ ) {
    ...
    double absoluteTol = fractionalTol*std::abs(x) + tinyAbsoluteTol;
    double absoluteTol_Times2 = absoluteTol * 2.0;
    double uncertaintyOfEstimate = std::abs(x-bracketMidpoint) + 0.5*( b - a );
    ...
    double numerator;
    double denominator;
    if ( std::abs(d_twoIterationsAgo) > absoluteTol ) {
      ...
      if ( std::abs(d) >= std::abs( 0.5*d_twoIterationsAgo ) || x+d <= a || x+d >= b || std::isnan(d) || std::isinf(d) ) {
    ...
    if ( std::abs(d) < absoluteTol ) d = ( d < 0.0 ) ? -absoluteTol : absoluteTol;
