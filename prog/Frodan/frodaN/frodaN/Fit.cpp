...
void Fit::setSourceBasePoints( const std::vector<Vec3> &base_, double radiusGyration_ ) {
  sourceAbsoluteCenter = Vec3(0,0,0);
  base = base_;
  rotor = Vec3( 0,0,0 );
  rotated = base;
...
void Fit::setSourceAbsolutePoints( const std::vector<Vec3> &absolute_ ) {
  ...
  for ( size_t i=0; i < N; i++ ) {
    sourceAbsoluteCenter += absolute_[i];
  }
  sourceAbsoluteCenter /= N;

  base.resize( N );
  for ( size_t i=0; i < N; i++ ) {
    base[i] = absolute_[i] - sourceAbsoluteCenter;
    rg += base[i].norm2();
  }
  rg = sqrt( rg/N );
...
void Fit::setSourceAbsolutePoints( const std::vector<Vec3> &absolute_, const std::vector<int>& subsetIndices ) {
  sourceAbsoluteCenter = Vec3(0,0,0);
  double rg = 0;
  size_t N = subsetIndices.size();
  for ( size_t i=0; i < N; i++ ) {
    sourceAbsoluteCenter += absolute_[ subsetIndices[i] ];
  }
  sourceAbsoluteCenter /= N;

  base.resize( N );
  for ( size_t i=0; i < N; i++ ) {
    base[i] = absolute_[ subsetIndices[i] ] - sourceAbsoluteCenter;
    rg += base[i].norm2();
  }
  rg = sqrt( rg/N );
...
void Fit::setTargetAbsolutePoints( const std::vector<Vec3> &absolute_, const std::vector<int>& subsetIndices ) {
  targetAbsoluteCenter = Vec3(0,0,0);
  size_t N = subsetIndices.size();
  for ( size_t i=0; i < N; i++ ) {
    targetAbsoluteCenter += absolute_[ subsetIndices[i] ];
  }
  targetAbsoluteCenter /= N;

  target.resize( N );
  for ( size_t i=0; i < N; i++ ) {
    target[i] = absolute_[ subsetIndices[i] ] - targetAbsoluteCenter;
  }
...
void Fit::setRotor( const Vec3& rotor_ ) {
  ...
  rotor = rotor_;
  currentRotation.setRotor( rotor );
  size_t N = base.size();
  for ( size_t i = 0; i < N; i++ ) {
...
void Fit::getFitAbsolutePoints( std::vector<Vec3>& fitPoints ) const {
  fitPoints = rotated;
  for ( size_t i = 0; i < fitPoints.size(); i++ ) {
    fitPoints[i] += targetAbsoluteCenter;
  }
}
