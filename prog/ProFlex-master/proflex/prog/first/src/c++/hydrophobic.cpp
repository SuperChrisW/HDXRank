...
int list::determine_vdWaals_contact( node *cp, node *tp) {
  ...
  if(atom1[2]== 'S') {vdr1 = 1.80;}
  else if(atom1[2]=='C') {
    vdr1 = 1.70;
    if(strcmp(atom1,"  CA ")) { 
       if(!strcmp(cp->r1.field1,"HETATM") ) {
	 if( !strcmp(cp->r1.rname," CA") || !strcmp(cp->r1.rname,"CA ") ){vdr1 = 0.0;}
       }
    }
  }
  if(atom2[2]== 'S') {vdr2 = 1.80;}
  else if(atom2[2]=='C') {
    vdr2 = 1.70;
    if(strcmp(atom2,"  CA ")) { 
       if(!strcmp(tp->r1.field1,"HETATM") ) {
	 if( !strcmp(tp->r1.rname," CA") || !strcmp(tp->r1.rname,"CA ") ){vdr2 = 0.0;}
       }
    }
  }
  ...
  /**************************************************/
  if( (atom_atom_dis < 2.20 ) ||
      (atom_atom_dis > ((sum_of_vdWaals) + R_factor)) )
    is_contact = 0;
...
int list::only_bonded_to_Hphobes(int so, int sf){
  ...
  for( a = 1; a <= mult[so]; a++ ) {
	...
	// AJR 06.24.03 added a line to omit hydrogen atoms as counting against - Sameer
...
int list::find_hydrophob(void) {
  ...
  /* ################################################################AJR 03.21.02 
     Identify the Hydrophobic contacts and place them into the array hp_tether[hpcount][i]
     ######################################################################################*/
	...
	for( jy =- 1; jy <= 1; jy++) {
	  for( jz =- 1; jz <= 1; jz++) {
		...
		if( (atype2[2]== 'C' || atype2[2]=='S') &&
		    ...
		    not_2nd_neighb(so,sf) &&
		    not_3rd_neighb(so,sf) &&
...
void list::HPaugment(node **node_p) {
  ...
  for(i=1; i<=no_tether;i++){
    ...
    for( a = 1; a < 4; a++ ){
      ...
      strcpy(fresh->r1.field1,"HETATM");      // PDB field identifier (ie. ATOM, HETATM, REMARK)
    ...
    //strcpy(string, line+66, 5 );          // some kind of internal FIRST identifier.  
...
void list::create_newPSEUDO(node *temp1, node *temp2) {
  ...
  *else {    cid = 'G';  }*/
  ...
  for( a = 1; a < 4; a++ ){
    ...
    //hp_points[*hp_point_counter][a] = current_atom.coords[a] + (0.5*unit_vector[a]);
...
//      hp_points[*hp_point_counter][a] = current_atom.coords[a] + (0.5*unit_vector[a]);
  ...
  //  (*hp_point_counter)++;
  /*
  fprintf( output_file, "HETATM%5d  X   XXX %c%4d     %7.3f %7.3f %7.3f  1.00  1.00      N\n", 
           *next_atom_num, cid, *residue_number, vector_one[0], vector_one[1], vector_one[2] );
  fprintf( output_file, "HETATM%5d  X   XXX %c%4d     %7.3f %7.3f %7.3f  1.00  1.00      N\n", 
           *next_atom_num+1, cid, *residue_number, vector_two[0], vector_two[1], vector_two[2]
);
  fprintf( output_file, "HETATM%5d  X   XXX %c%4d     %7.3f %7.3f %7.3f  1.00  1.00      N\n", 
           *next_atom_num+2, cid, *residue_number, vector_three[0], vector_three[1],vector_three[2] );
  (*residue_number)++;
*/
}
...
void list::write_HPCFbond(void) {
  ...
  for(i=1; i<=no_tether;i++) {
    ...
    xofil<<"REMARK:CF"<<setw(6)<<latom+1<<setw(6)<<latom+2<<endl;
    latom=latom+2;
  }
...
void list::write_HPHBbond(void) {
  ...
  for(i=1; i<=no_tether;i++){
    ...
    //    so = hp_tether[i][0];
    sf = hp_tether[i][1];
    xofil<<"REMARK:HB"<<setw(5)<<ahb+i<<setw(13)<<setprecision(6)<<ephob
	 <<setw(8)<<patom<<setw(8)<<patom+1<<setw(8)<<sf<<"    PH hydr phob"<<endl;
    patom++;
  }
...
void list::write_HPatom(node **node_p) {
  ...
  for(i=1; i<=no_tether;i++){
    ...
    //  node_p[so]->r1.coord[1]
      unit_vector[a]  = node_p[so]->r1.coord[a+1] - node_p[sf]->r1.coord[a+1];
    ...
    jt++;
    xofil<<"HETATM"<<setw(5)<<nps+jt<<"  X   XXX T"<<setw(4)<<rescount
	 <<setw(12)<<setprecision(3)
	 << setiosflags(ios::showpoint | ios::fixed | ios::right)
	 << vector_one[0]	 <<setw(8)<<setprecision(3)
	 <<vector_one[1]<<setw(8)<<setprecision(3)<<vector_one[2]
	 <<"  0.00  0.00      N"<<endl;
    jt++;
    xofil<<"HETATM"<<setw(5)<<nps+jt<<"  X   XXX T"<<setw(4)<<rescount
	 <<setw(12)<<setprecision(3)<<vector_two[0]<<setw(8)<<setprecision(3)
	 <<vector_two[1]<<setw(8)<<setprecision(3)<<vector_two[2]
	 <<"  0.00  0.00      N"<<endl;
    jt++;
    xofil<<"HETATM"<<setw(5)<<nps+jt<<"  X   XXX T"<<setw(4)<<rescount
	 <<setw(12)<<setprecision(3)<<vector_three[0]<<setw(8)<<setprecision(3)
	 <<vector_three[1]<<setw(8)<<setprecision(3)<<vector_three[2]
	 <<"  0.00  0.00      N"<<endl;
    rescount++;
  }
