...
c If the atom so has exactly one nearest neighbor (multref(so) .eq. 1), 
c it sets the cluster label of so (clst(so)) to be the same as the cluster label of 
c its neighbor sf (clst(sf)), where sf is the first nearest neighbor to the site so.
...
c     ---------------------------------------------- count BULK sites in cluster nco
            ...
            do jo=1,multref(so)
               indexo = ipo + jo
               sf = linkref(indexo)
c  sf each neighbor sf of the atom so
...
c            size(nc) = size(nc) + multarc(nc)
c            size(nc) = size(nc) + multbrc(nc)
            size(nc) = multarc(nc)
c     ------------------------------------------------------------ find maximum size 
...
c     --------------------------------------------- calculate cluster size histogram 
         ...
         open(unit=10, file='clst_output.txt', status='unknown')
      ! Write the clst array to the file
         do i = 1, natom
            write(10,*) i, aname(i), clst(i)
         end do
c         do s=1,maxsize
c            if( oldlabel(s) .ne. 0 ) write(10,*) s,oldlabel(s)
c         end do
