...
def generate_embedding(hhm_file, dssp_file, output_file):
...
for id, row in df.iterrows():
    ...
    file = row['file']
    if os.path.isfile(f'{root_dir}/embedding_files/{uniprot_id}.embedding.npy'):
        continue
    try:
        print(uniprot_id)
        dssp_file = os.path.join(dssp_dir, f'{uniprot_id}.dssp.txt')
        hhm_file = os.path.join(hhm_dir, f'{file}.hhm')

        if os.path.isfile(dssp_file) and os.path.isfile(hhm_file):
            generate_embedding(hhm_file, dssp_file, f'{root_dir}/embedding_files/{uniprot_id}.embedding.txt')
