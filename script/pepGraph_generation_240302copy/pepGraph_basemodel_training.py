...
def prepare_data(root_dir, embedding_file, pdb, chain,
    ...
    input_array, truth_array, start_pos, end_pos, log_t = \
                                        seq_embedding(target_HDX_file, embedding_file, protein, state, db, chain,
...
def base_data(hdx_df, root_dir):
    ...
    for row_id, row in hdx_df.iterrows():
        ...
        if not returned_value:
            continue
        input_array, truth_array = returned_value
        if np.isnan(truth_array).any():
            print('nan value found')
            continue
        truth_array = np.array(truth_array)/100
        
        input_data.extend(input_array)
        truth_data.extend(truth_array)

    input_data = np.array(input_data)
    truth_data = np.array(truth_data)
    
    return input_data, truth_data
